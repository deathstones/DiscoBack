...
info_request = requests.get(URLs.me, headers=headers)
...
with open(f"{backup_path}\\self.txt", "w", encoding="utf-8") as file:
    file.write(f"Username: {info_json['username']}\nGlobal Name: {info_json['global_name']}\nEmail: {info_json['email']}\nPhone: {info_json['phone']}\nBio:\n{info_json['bio']}")
print("[+] Backed up basic info.")

if info_json['banner'] is not None:
    open(f"{backup_path}\\banner.gif", "wb").write(requests.get(f"https://cdn.discordapp.com/avatars/{info_json['id']}/{info_json['banner']}.gif?size=4096").content)
    print("[+] Backed up profile banner.")
if info_json['avatar'] is not None:
    open(f"{backup_path}\\pfp.png", "wb").write(requests.get(f"https://cdn.discordapp.com/avatars/{info_json['id']}/{info_json['avatar']}.png?size=1024").content)
    print("[+] Backed up profile picture.")

# Backup friends list.
friends_list = requests.get(URLs.relationships, headers=headers).json()
with open(f"{backup_path}\\friends.txt", "w", encoding="utf-8") as f:
    i = 0
    f.write(f"ID - Username - Display Name - Nickname\n")
    for entry in friends_list:
        i+=1
        user_info = entry['user']
        if i is len(friends_list):
            f.write(f"{user_info['id']} - {user_info['username']} - {user_info['global_name']} - {entry['nickname']}")
        else:
            f.write(f"{user_info['id']} - {user_info['username']} - {user_info['global_name']} - {entry['nickname']}\n")
print(f"[+] Backed up {len(friends_list)} friends.")

# Backup joined guilds.
guild_list = requests.get(URLs.guilds, headers=headers).json()
with open(f"{backup_path}\\guilds.txt", "w", encoding="utf-8") as f:
    i = 0
    f.write(f"ID - Name - Owner\n")
...
open(f"{backup_path}\\settings.txt", "w", encoding="utf-8").write(requests.get(URLs.settings, headers=headers).text)
...
# Zipping the folder.
shutil.make_archive(f"{og_backup_path}/Discord Backup @ {backup_time}", 'zip', backup_path)
print(f"[+] Zipped up backup to Discord Backup @ {backup_time}.")

# Deleting the normal folder.
shutil.rmtree(backup_path)
print(f"[+] Deleted original backup folder.")